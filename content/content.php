<?php 

    return [
        'home' => [
            'meta-title' => 'Nanolash strona główna title',
            'meta-description' => 'Nanolash strona główna description',
            'title' => 'EKSPERT W DZIEDZINIE <br><strong>PIĘKNYCH RZĘS</strong>',
            'description' => 'Poznaj świetnej jakości produkty do przedłużania i pielęgnacji rzęs oraz akcesoria do ich stylizacji.',
            'subtitle' => '<span>DLACZEGO</span> NANOLASH?',
            'subdescriotion' => '<span class="shadow">Naturalne rzęsy zachwycające długością, intensywnym kolorem i spektakularną gęstością? To możliwe! Wystarczy miesiąc, by całkowicie odmienić Twoje spojrzenie.</span>',
            'brands' => [
                'nanolash' => [
                    'classes' => 'left dark',
                    'title' => 'Marka NANOLASH',
                    'content' => '<p>Poznaj luksusową serię produktów do rzęs Nanolash. Wybierz doskonałej jakości sztuczne rzęsy, kosmetyki do przedłużania i pielęgnacji, akcesoria do stylizacji, a także makijażu rzęs. W naszej ofercie znajdują się preparaty do pielęgnacji, laminacji i liftingu oraz malowania, a także kultowa już odżywka na porost i wzmocnienie rzęs. </p>',
                    'icons' => [
                        0 => 'Lashes with 25+ Uses',
                        1 => 'Premium, Innovative Lash Fibres',
                        2 => '100% Vegan',
                        3 => '100% Recyclable Packaging'
                    ]
                ],
                'nanobrow' => [
                    'classes' => 'right light',
                    'title' => 'Marka NANOBROW',
                    'content' => '<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum</p>',
                    'icons' => [
                        0 => 'Lashes with 25+ Uses',
                        1 => 'Premium, Innovative Lash Fibres',
                        2 => '100% Vegan',
                        3 => '100% Recyclable Packaging'
                    ]
                ]
            ],
            'grid' => [
                'nanolash-es' => [
                    'title' => 'Eyelash Serum',
                    'description' => 'Profesjonalna odżywka do brwi',
                ],
                'nanolash-mp' => [
                    'title' => 'Mascara Primer',
                    'description' => 'Tusz do Rzęs Zwiększający Objętość',
                ],
                'nanolash-maskara' => [
                    'title' => 'Mascara',
                    'description' => 'Tusze do Rzęs',
                ],
                'nanolash-labs' => [
                    'title' => 'Lash & Brow Shampoo',
                    'description' => 'Szampon do rzęs i brwi',
                ],
                'nanolash-diy' => [
                    'title' => 'DIY Eyelash Extensions',
                    'description' => 'Tusz do Rzęs Wydłużający i Podkręcający',
                ],
                'nanolash-llk' => [
                    'title' => 'Lash Lift Kit',
                    'description' => 'Tusz do Rzęs Zwiększający Objętość',
                ],
                'nanolash-vl' => [
                    'title' => 'Volume Lashes',
                    'description' => 'Kępki do makijażu',
                ],
                'nanolash-hep' => [
                    'title' => 'Hydrogel Eye Patches',
                    'description' => 'Hydrogel Eye Patches',
                ],
                'nanolash-dmw' => [
                    'title' => 'Disposable Mascara Wands',
                    'description' => 'Wands',
                ],
            ],
            'details' => [
                'products' => [
                    'nanolash-es' => [
                        'title' => 'Nanolash Eyelash Serum',
                        'description' => 'Profesjonalna odżywka do brwi',
                    ],
                    'nanolash-pv' => [
                        'title' => 'Nanolash Primer Mascara',
                        'description' => 'Tusz do Rzęs Zwiększający Objętość',
                    ],
                    'nanolash-vm' => [
                        'title' => 'Nanolash Volume Up Mascara',
                        'description' => 'Tusz do Rzęs Zwiększający Objętość',
                    ],
                    'nanolash-lcm' => [
                        'title' => 'Nanolash Lenght & Curl Mascara',
                        'description' => 'Tusz do Rzęs Wydłużający i Podkręcający',
                    ],
                    'nanolash-lbs' => [
                        'title' => 'Nanolash Lash & Brow Shampoo',
                        'description' => 'Kleje do przedłużania rzęs',
                    ],
                    'nanolash-hyp' => [
                        'title' => 'Nanolash Hydrogel Eye Patches',
                        'description' => 'Kępki do makijażu',
                    ],
                ]
            ],
            'faq' => [
                'title' => '<strong>FAQ</strong> WSZYSTKO <br> CO POWINNAŚ WIEDZIEĆ',
                'questions' => [
                    [
                        'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                        'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                    ],
                    [
                        'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                        'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                    ],
                    [
                        'question' => 'Ile trwa realizacja zamówienia?',
                        'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                    ],
                    [
                        'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                        'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                    ],
                    [
                        'question' => 'Jakie są sposoby płatności za zamówienie?',
                        'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                    ],
                ] 
            ],

        ],
        'products' => [
            'nanolash-es' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'product_data' => [
                    'capacity' => ['Pojemność: ' => '5ml / 0.17 fl oz']
                ],
                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'content_1' => [
                    'img_alt' => 'test alt',
                    'section_classes' => 'product-content container d-md-grid',
                    'content_classes' => 'product-content__bg-image d-none d-md-block',
                    'content_1_1' => '
                        <h2 class="section-title full">Podkreśl swoje<br>
                        <strong>naturalne piękno!</strong></h2>
                        <div class="first">
                            <p>Naturalnie <strong>gęste brwi</strong> w <strong>niecały miesiąc</strong>? To możliwe! Odżywka na porost brwi Nanobrow ma wszystko, czego potrzebują Twoje brwi, by rosnąć szybciej i wyglądać piękniej.</p>
                            <h3 class="section-subtitle">Jak to możliwe?</h3>
                            <p>Nanobrow podkreśla naturalne brwi, przyspiesza ich wzrost i hamuje nadmierne wypadanie, dzięki czemu łuk brwiowy <strong>wygląda perfekcyjnie nawet bez makijażu</strong>. Odżywka do brwi działa podobnie jak w przypadku rzęs. Rozwiązuje problem cienkich, rzadkich i niewyraźnych brwi u źródła, czyli dostarczając <strong>wszystko to, co jest niezbędne do regeneracji i zachowania pięknego wyglądu</strong>.</p>
                        </div>',
                    'content_1_2' => '
                        <div class="full">
                            <h3 class="section-subtitle">Nanobrow Eyebrow Serum — najlepszy wybór</h3>
                            <p>Nanobrow to odżywka, którą pokochały kobiety! Łatwa aplikacja, lekka formuła, która nie obciąża brwi i <strong>zjawiskowe efekty</strong> – to wszystko sprawia, że Nanobrow zbiera <strong>pozytywne recenzje</strong>.</p>
                            <p>Ty też możesz dołączyć do grona tych kobiet, które oddały swoje brwi pod opiekę Nanobrow. <strong>Sprawdź, jak piękne mogą być Twoje brwi</strong> i zyskaj więcej pewności siebie dzięki <strong>idealnie podkreślonemu spojrzeniu</strong>. 
                            </p>
                        </div>' 
                ],
                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],
                'gallery' => [
                    'extra' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>'
                ],
                'content_2' => [
                    'img_alt' => 'test gif alt',
                    'section_classes' => 'product-content product-content__wrapper container d-grid',
                    'content_1_1' => '
                    <h2 class="product-content__title section-title"><span><strong>Stylizacja brwi</strong></span> prostsza niż myślisz</h2>
                    <div class="product-content__content">
                        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Amet inventore vel consequuntur, temporibus voluptatem recusandae quia nobis libero minus asperiores quas saepe sint perspiciatis accusantium voluptate laboriosam placeat tenetur nesciunt.</p>
                        <h3 class="section-subtitle">Jak to możliwe?</h3>
                        <p>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ea earum doloremque aspernatur libero. Provident autem alias molestiae esse necessitatibus, illo cupiditate exercitationem facere culpa, explicabo corporis. Ipsum voluptatem excepturi praesentium? Harum commodi itaque, ea dolores sint cum sed. Aperiam deserunt magnam quaerat modi fugiat ipsa eos corrupti in qui eligendi, iure doloribus laborum sunt aut harum voluptates tenetur quas quam.</p>
                    </div>',
                    'content_1_2' => '
                        <div class="product-content__extra">
                            <h3 class="section-subtitle">Nanobrow Eyebrow Serum — Lorem, ipsum.</h3>
                            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Hic nisi quaerat illum assumenda, deleniti eligendi saepe accusantium provident dicta rem quasi. Inventore quo deserunt ea iste perferendis, harum obcaecati nobis molestiae, praesentium commodi quaerat, blanditiis exercitationem voluptatibus facere eligendi ex.</p>
                            <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Corrupti facere debitis impedit, labore minus quasi numquam in soluta eos itaque.</p>
                        </div>
                    ',
                ],
                'content_3' => [
                    'img_alt' => 'test gif alt',
                    'section_classes' => 'product-content product-content__wrapper--reverse container d-grid',
                    'content_1_1' => '
                        <h2 class="product-content__title section-title"><span><strong>Stylizacja brwi</strong></span> prostsza niż myślisz</h2>
                        <div class="product-content__content">
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Amet inventore vel consequuntur, temporibus voluptatem recusandae quia nobis libero minus asperiores quas saepe sint perspiciatis accusantium voluptate laboriosam placeat tenetur nesciunt.</p>
                            <h3 class="section-subtitle">Jak to możliwe?</h3>
                            <p>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ea earum doloremque aspernatur libero. Provident autem alias molestiae esse necessitatibus, illo cupiditate exercitationem facere culpa, explicabo corporis. Ipsum voluptatem excepturi praesentium? Harum commodi itaque, ea dolores sint cum sed. Aperiam deserunt magnam quaerat modi fugiat ipsa eos corrupti in qui eligendi, iure doloribus laborum sunt aut harum voluptates tenetur quas quam.</p>
                        </div>',
                    'content_1_2' => '',
                ],
                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-vl' => [
                'title' => 'Najlepsze kępki do rzęs',
                'description' => 'Poznaj najlepsze kępki do rzęs w całym Radomiu',
                'slogan' => 'Najlepsze kępki do rzęs',
                'mark_name' => 'Nanolash',
                'product_name' => 'Rzęsy do przedłużania',
                'product_variants' => [
                    [
                        'key' => 'skręt', 'values' => ['C' => 'C', 'D' => 'D']
                    ],
                    [
                        'key' => 'grubość', 'values' => ['005' => '0.05', '007' => '0.07', '010' => '0.10', '012' => '0.12', '015' => '0.15']
                    ],
                    [
                        'key' => 'długość', 
                        'values' => [
                            'M0613' => 'Mixed 6mm-13mm',
                            '09' => '9mm',
                            '10' => '10mm',
                            '11' => '11mm',
                        ]
                    ],
                ],
                'icons_section' => [
                    'title' => 'NANOLASH <strong>RZĘSY DO PRZEDŁUŻANIA</strong>',
                    'icons' => [
                        'Wysoka temperatura od 70 ℃ do 100 ℃',
                        'Miękkie i lekkie',
                        'Głęboka czerń z matowym wykończeniem',
                        'Grubość od 0,05 do 0,15 mm'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>LOREM IPSUM</span> <strong>DOLOR SIT AMET, CONSECTETUR</strong>',
                    'content' => '<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</p>
                    <p>Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum</p>'
                ],
                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],
                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
            ],
            'nanolash-mp' => [
                'title' => 'title Mascara Primer',
                'description' => 'description Mascara Primer',

                'mark_name' => 'Nanolash',
                'product_name' => 'Mascara Primer',
                'slogan' => 'Coś tam mascara',
                'product_data' => [
                    'color' => ['Kolor: ' => 'transparentny'],
                    'formula' => ['Formula: ' => 'żelowa'],
                    'capacity' => ['Pojemność: ' => '10ml']
                ],
                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Mascara Primer?',
                    'icons' => [
                        'pogrubienie, wydłużenie i podkręcenie rzęs w jednym',
                        'intensywna codzienna pielęgnacja',
                        'wzorowe rozdzielenie rzęs',
                        'poprawa trwałości tuszu'
                    ],
                ],
                'product_effects' => [
                    'title' => '<strong>Z bazą możesz więcej!</strong>',
                    'content' => '<p>Czy doskonały tusz może być jeszcze lepszy? Tak! Dodaj swoim rzęsom mocy jeszcze zanim pokryjesz je ulubioną mascarą. Dzięki primerowi pod tusz Nanolash odkryjesz, jak wielki potencjał kryją Twoje rzęsy. Każdy tusz zyska dzięki niemu niesamowitą poprawę parametrów. Co może zrobić dla Ciebie baza do rzęs Nanolash? Sprawi, że rzęsy będą wydawały się dłuższe, grubsze, bardziej uniesione i wyjątkowo podkręcone. Pokrycie ich tuszem podkreśli ten efekt, a do tego zyska lepszą przyczepność, co przekłada się na jego trwałość i wygodę podczas aplikowania.</p>',
                ],
                'content_1' => [
                    'img_alt' => 'test alt',
                    'section_classes' => 'product-content container d-md-grid',
                    'content_classes' => 'product-content__bg-image d-none d-md-block',
                    'content_1_1' => '
                        <h2 class="section-title full">Podkreśl swoje<br>
                        <strong>naturalne piękno!</strong></h2>
                        <div class="first">
                            <p>Baza Nanolash zawiera szereg przemyślanie dopasowanych substancji odżywczych, które odznaczają się wielokierunkowym działaniem. Kompozycja opracowana przez Nanolash sprawia, że przy regularnym stosowaniu primera do rzęs pod makijaż rzęsy stają się zauważalnie mocniejsze i odporniejsze na uszkodzenia. Zauważysz także, że są dłuższe, grubsze i zachwycają naturalnym blaskiem. Jakie fenomenalne składniki znajdują się w primerze Nanolash?</p>
                            <p>DUPLIKAT!!! Baza Nanolash zawiera szereg przemyślanie dopasowanych substancji odżywczych, które odznaczają się wielokierunkowym działaniem. Kompozycja opracowana przez Nanolash sprawia, że przy regularnym stosowaniu primera do rzęs pod makijaż rzęsy stają się zauważalnie mocniejsze i odporniejsze na uszkodzenia. Zauważysz także, że są dłuższe, grubsze i zachwycają naturalnym blaskiem. Jakie fenomenalne składniki znajdują się w primerze Nanolash?</p>
                        </div>',
                    'content_1_2' => '
                        <div class="full" style="margin-bottom: 20px">
                            <h3 class="section-subtitle">Wszystko w wymiarze maxi</h3>
                            <p>Praktyczny kształt wyprofilowanej silikonowej szczoteczki to piękny kształt Twoich rzęs. Jeszcze przed nałożeniem tuszu będą one zauważalnie bardziej uniesione, podkręcone, wydłużone i pogrubione. Bezbarwnym primerem szybko i wygodnie pokryjesz nawet najkrótsze rzęsy w kącikach i wydobędziesz z nich maksimum potencjału.</p>
                            <p>Co daje baza pod tusz do rzęs Nanolash? Kluczowym działaniem bazy pod mascarę jest poprawa właściwości oferowanych przez wybrany przez Ciebie tusz. Transparentna formuła nie ingeruje w jego kolor, za to podnosi przyczepność pigmentu, co przekłada się na dłuższą trwałość makijażu i eliminację takich problemów, jak odbijanie się mascary na powiekach, powstawanie grudek i kruszenie oraz rozmazywanie.</p>
                            <p>Regularnie stosowana baza pod tusz do rzęs Nanolash skutecznie działa jako aktywator wzrostu rzęs, realnie przekładająca się na ich wzmocnienie i wydłużenie.</p>
                        </div>
                    ' 
                ],
                'gallery' => [
                    'extra' => '',
                ],
                'customers__opinions' => [
                    [
                        'comment' => ' Mam naturalnie bardzo ciemne rzęsy, więc zdarza mi się po prostu używać jej zamiast tuszu – widzę dużą różnicę w wyglądzie i kondycji moich rzęs. Boska!',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Spektakularnie wydłuża i pogrubia, na pewno kupię kolejne opakowanie. Najbardziej podoba mi się w tym primerze to, że faktycznie tusz trzyma się lepiej i nie muszę się przejmować poprawkami.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Ten podkład pod tusz do rzęs poleciła mi moja kosmetyczka. Jestem więcej, niż zadowolona, już po pierwszej aplikacji i pokryciu tuszem zauważyłam różnicę.',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Super wegański skład, szybko się wchłania, nie drażni moich wrażliwych oczu. Mój ulubiony primer!',
                        'author' => 'Anna, 35 lat'
                    ],
                    [
                        'comment' => 'To najlepsza baza pod tusz, z jakiej miałam przyjemność korzystać. Pierwsze efekty widać od razu, a na efekty pielęgnacji trzeba trochę poczekać, ale zdecydowanie warto! ',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Wygodna w użyciu szczoteczka i przyjemna, formuła, która nie spływa i nie podrażnia. Już po pierwszym użyciu przekonałam się, że tusz trzyma się lepiej. Po dłuższym, codziennym nakładaniu bazy pod mascarę zauważyłam poprawę wyglądu i kondycji rzęs. Zdecydowanie polecam.',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Najbardziej w tym produkcie podoba mi się zauważalne działanie pielęgnacyjne. Doskonałe uzupełnienie serum podbijające efekt każdego tuszu, jaki testowałam z tą bazą.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],
                'gallery' => '',
                'content_2' => [
                    'img_alt' => 'test gif alt',
                    'section_classes' => 'product-content product-content__wrapper container d-grid',
                    'content_1_1' => '
                    <h2 class="product-content__title section-title"><span><strong>Jak używać</strong></span> bazy pod tusz?</h2>
                    <div class="product-content__content">
                        <p>Primer nakładaj na oczyszczone rzęsy. Zacznij od samej nasady rzęs i delikatnie przeczesuj je szczoteczką, kierując się ku górze. Jedna warstwa produktu w zupełności wystarczy. Odczekaj chwilę i pozwól bazie wyschnąć przed nałożeniem tuszu.</p>
                        <p>Chcesz uzyskać jeszcze bardziej spektakularne efekty? Zamień zwykły tusz na jedną z maskar Nanolash! Formuły tuszy i primera Nanolash doskonale uzupełniają się wzajemnie i razem pozwalają uzyskać iście zniewalający wachlarz rzęs, którego efektami bez poprawek będziesz mogła cieszyć się przez cały dzień!</p>
                    </div>',
                    'content_1_2' => '
                        <div class="product-content__extra">
                            <p>Wybierz Volume Up Mascara, jeśli zależy Ci przede wszystkim na fenomenalnym pogrubieniu i zwiększeniu objętości lub Nanolash Lenght & Curl Mascara, jeśli Twoim priorytetem jest wydłużenie i podkręcenie naturalnych rzęs.</p>
                        </div>
                    ',
                ],
                'content_3' => [
                    'img_alt' => 'test gif alt',
                    'section_classes' => 'product-content product-content__wrapper--reverse container d-grid',
                    'content_1_1' => '
                        <h2 class="product-content__title section-title"><span><strong>Nanolash – </strong></span> wszystko, co najlepsze dla Twoich rzęs</h2>
                        <div class="product-content__content">
                            <p>Baza pod tusz do rzęs jest zaledwie jednym z rewelacyjnych produktów, jakie znajdziesz w ofercie Nanolash. To najwyższej jakości kosmetyki do pielęgnacji i makijażu, po które chętnie sięgają również profesjonalne stylistki rzęs na całym świecie.</p>
                            <p>Jeśli chcesz zapoznać się z nowościami w branży beauty i znaleźć rzetelne źródło porad oraz niewyczerpane źródło inspiracji, koniecznie odwiedź nasze media społecznościowe!</p>
                        </div>',
                    'content_1_2' => '',
                ],
                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Co daje baza pod tusz do rzęs?',
                            'answer' => 'Wiele osób zastanawia się, czy baza pod tusz jest potrzebna. Warto po nią sięgnąć, by spotęgować działanie maskary. Dzięki temu rzęsy prezentują się jeszcze bardziej spektakularnie! Co więcej: dobra baza pod tusz przedłuża jego trwałość, a także dba o odżywienie i wzmocnienie rzęs. Jak widać, primer pod tusz ma wiele zalet!',
                        ],
                        [
                            'question' => 'Czy można stosować Nanolash Mascara Primer z soczewkami?',
                            'answer' => 'Tak. Primer pod tusz Nanolash jest odpowiedni dla osób noszących szkła kontaktowe.',
                        ],
                        [
                            'question' => 'Czy Nanolash Mascara Primer jest odpowiednia dla oczu wrażliwych?',
                            'answer' => 'Jak najbardziej! baza pod tusz Nanolash może być stosowana przez osoby o wrażliwej skórze wokół oczu lub wrażliwych oczach. ',
                        ],
                        [
                            'question' => 'Jak długo Nanolash Mascara Primer utrzymuje się na rzęsach?',
                            'answer' => 'Nanolash Mascara Primer przedłuża trwałość makijażu, więc możesz być pewna, że Twój make-up będzie prezentował się doskonale przez cały dzień lub noc– dopóki nie zdecydujesz, że pora go zmyć z rzęs!',
                        ],
                        [
                            'question' => 'Jaki jest okres przydatności primera do rzęs Nanolash?',
                            'answer' => 'Przy odpowiednim przechowywaniu nasza baza pod tusz zachowuje swoje właściwości do 6 miesięcy.',
                        ],
                        [
                            'question' => 'Czy baza pod tusz musi wyschnąć?',
                            'answer' => 'Aby zapewnić jak najlepszy efekt w makijażu, poczekaj aż primer pod tusz wyschnie i dopiero pomaluj rzęsy maskarą.',
                        ],
                        [
                            'question' => 'Ile warstw bazy pod tusz powinnam nałożyć?',
                            'answer' => 'Nanolash Mascara Primer jest bardzo wydajna! Jedna, cienka warstwa bazy nałożona pod tusz w zupełności wystarczy, by zapewnić sobie olśniewający makijaż rzęs! ',
                        ],
                        [
                            'question' => 'Jaki jest skład Nanolash Mascara Primer?',
                            'answer' => 'INCI: Aqua, Propanediol, Carbomer, Arginine, Lactic Acid, Glycine Soja Germ Extract, Triticum Vulgare Germ Extract, Scutellaria Baicalensis Root Extract, Sodium Benzoate, Gluconolactone, Calcium Gluconate, Phenoxyethanol, Ethylhexylglycerin, Triethanolamine, Panthenol, Caffeine.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Przewidywany okres realizacji zamówienia wynosi dwa dni robocze. Produkt doręczany jest przez firmę kurierską.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, jeśli mieszkam za granicą?',
                            'answer' => 'Tak, sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w prawym górnym rogu strony.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-lacm' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-vum' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-labs' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-diy-heartbreaker' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-diy-charm' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-diy-innocent' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-diy-fantasy' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-diy-classy' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-diy-devine' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-diy-harmony' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-diy-flirty' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-llk' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-hep' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-dmw' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-maa' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
            'nanolash-lfa' => [
                'title' => 'Najlepszy Serum do rzęs',
                'description' => 'Poznaj najlepszy serum do rzęs w całym Radomiu - Nanolash',

                'mark_name' => 'Nanolash',
                'product_name' => 'Eyelash Serum',
                'slogan' => 'Profesjonalna odżywka do rzęs',
                'capacity' => 'Pojemność: ',
                'capacity_value' => '5ml / 0.17 fl oz',

                'icons_section' => [
                    'title' => '<span><strong>Jak działa</strong> Nanolash</span> Eyelash Serum?',
                    'icons' => [
                        'stylizuje i podkreśla naturalny kształt brwi',
                        'delikatnie maluje i uwydatnia piękno Twoich brwi',
                        'dyscyplinuje i utrwala włoski przez 24 h',
                        'olejek rycynowy wzmacnia i nadaje połysku'
                    ],
                ],
                'product_effects' => [
                    'title' => '<span>Odżywka Nanolash –</span> <strong>długie, gęste i piękne rzęsy!</strong>',
                    'content' => '<p>Jeśli pragniesz ułożonych, zadbanych, a przy tym naturalnie wyglądających brwi, Nanobrow Stylist Gel jest dla Ciebie! Jego dopracowana formuła w połączeniu z wygodnym aplikatorem sprawiają, że brwi są dobrze ułożone i wyglądają nenagannie przez cały dzień.</p>
                    <p>Nanobrow Stylist Gel jest trwały i precyzyjny: z łatwością modeluje i utrwala nawet najbardziej niesforne włoski! Olejek rycynowy działa na brwi odżywczo: wzmacnia i chroni przed niszczeniem. Jest wiele sposobów na piękne brwi, ale tylko jeden działa tak świetnie!</p>',
                ],
                'gallery' => '<p>Perfekcyjnemu efektowi żelowej formuły do brwi Nanobrow zaufały już tysiące kobiet na całym świecie, które są zgodne: to najlepszy żel do układania i pielęgnacji brwi! Ceniona za wygodę i precyzję w makijażu, oraz efekt elegancko i naturalnie wyglądających brwi. Pielęgnujący olejek rycynowy wzmacnia i upiększa brwi.</p>',

                'customers__opinions' => [
                    [
                        'comment' => ' Rzadko recenzuję cokolwiek, ale dla Nanobrow robię wyjątek, bo dzięki temu serum nie musiałam robić microbladingu! Moje własne brwi są teraz naturalnie gęste: wystarczy je tylko raz na jakiś czas wyregulować. Zyskałam pewność siebie i oszczędziłam.',
                        'author' => 'Sabina, 27 lat'
                    ],
                    [
                        'comment' => 'Używam Nanobrow od kilku miesięcy i jestem zachwycona efektami! Moje brwi są teraz pełniejsze i bardziej wyraziste. Polecam każdej kobiecie, która chce poprawić wygląd swoich brwi.',
                        'author' => 'Magdalena, 32 lata'
                    ],
                    [
                        'comment' => 'Nanobrow to prawdziwy game changer w mojej codziennej rutynie pielęgnacyjnej. Moje brwi wyglądają teraz znacznie lepiej, a ja czuję się pewniejsza siebie. Dziękuję!',
                        'author' => 'Katarzyna, 29 lat'
                    ],
                    [
                        'comment' => 'Zawsze miałam problem z cienkimi brwiami. Dzięki Nanobrow moje brwi stały się gęstsze i ciemniejsze. Jestem bardzo zadowolona z rezultatów i na pewno będę kontynuować stosowanie.',
                        'author' => 'Anna, 35 lat'
                    ],
                ],

                'faq' => [
                    'title' => '',
                    'questions' => [
                        [
                            'question' => 'Czy Nanolash mogą stosować osoby noszące szkła kontaktowe?',
                            'answer' => 'Produkty Nanolash mogą być stosowane przez osoby noszące soczewki kontaktowe. Można stosować je bez obaw, ponieważ ich skład został przebadany przez ekspertów',
                        ],
                        [
                            'question' => 'Mam bardzo wrażliwe oczy. Czy mogę używać kosmetyków Nanolash?',
                            'answer' => 'Wszystkie kosmetyki do rzęs Nanolash to gwarancja bezpieczeństwa. Ich działanie jest łagodne dla oczu i skóry, a jednocześnie dają spektakularne efekty.',
                        ],
                        [
                            'question' => 'Ile trwa realizacja zamówienia?',
                            'answer' => 'Produkt dostarczany jest za pośrednictwem firmy kurierskiej. Standardowy czas realizacji zamówienia to dwa dni robocze, ale może się on wydłużyć w wyjątkowych przypadkach.',
                        ],
                        [
                            'question' => 'Czy mogę złożyć zamówienie, mieszkając za granicą?',
                            'answer' => 'Tak, produkty Nanobrow sprzedajemy nie tylko w Polsce, ale także w wielu krajach na całym świecie. Koszt wysyłki i szacowany czas dostawy zależy od kraju dostawy. W razie potrzeby wybierz odpowiednią wersję językową dla swojego kraju, klikając na przycisk z flagą w menu strony.',
                        ],
                        [
                            'question' => 'Jakie są sposoby płatności za zamówienie?',
                            'answer' => 'Za każde zamówienie możesz zapłacić szybko i bezpiecznie przelewem lub kartą przez wybrany system online. Zamówione produkty zostaną do Ciebie wysłane w ciągu 24 godzin od zaksięgowania wpłaty na naszym koncie. Możesz też wybrać płatność przy odbiorze. Wówczas produkty wyślemy niezwłocznie po złożeniu zamówienia, a należną kwotę przekażesz kurierowi, odbierając przesyłkę.',
                        ],
                    ]
                ],
                    

            ],
        ],

        'categories' => [
            'nanolash-diy' => [
                'meta-title' => 'Nanolash DIY Eyelash Extensions title',
                'meta-description' => 'Nanolash DIY Eyelash Extensions description',
                'grid' => [
                    "nanolash-diy-charm" => [
                        'title' => 'DIY Eyelash Extensions',
                        'description' => 'Heartbreaker',
                    ],
                    "nanolash-diy-innocent" => [
                        'title' => 'DIY Eyelash Extensions',
                        'description' => 'Charm',
                    ],
                    "nanolash-diy-fantasy" => [
                        'title' => 'DIY Eyelash Extensions',
                        'description' => 'Innocent',
                    ],
                    "nanolash-diy-classy" => [
                        'title' => 'DIY Eyelash Extensions',
                        'description' => 'Fantasy',
                    ],
                    "nanolash-diy-devine" => [
                        'title' => 'DIY Eyelash Extensions',
                        'description' => 'Classy',
                    ],
                    "nanolash-diy-harmony" => [
                        'title' => 'DIY Eyelash Extensions',
                        'description' => 'Devine',
                    ],
                    "nanolash-diy-flirty" => [
                        'title' => 'DIY Eyelash Extensions',
                        'description' => 'Flirty',
                    ],
                ],
            ],
            'nanolash-maskara' => [
                'meta-title' => 'meta title do kategorii maskar',
                'meta-description' => 'meta description do kategorii maskar',
                'title' => 'Nanolash Mascara',
                'grid' => [
                    'nanolash-lacm' => [
                        'title' => 'LENGHT&CURL MASCARA',
                        'description' => 'cos tam cos tam mascara',
                    ],
                    'nanolash-vum' => [
                        'title' => 'VOLUME UP MASCARA',
                        'description' => 'cos tam cos tam mascara',
                    ],
                ],
                'content_1' => [
                    'img_alt' => 'test alt',
                    'section_classes' => 'product-content container mt',
                    'content_classes' => '',
                    'content_1_1' => '<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. </p>
                    <p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui ratione voluptatem sequi nesciunt. Neque porro quisquam est, qui dolorem ipsum quia dolor sit amet, consectetur, adipisci velit, sed quia non numquam eius modi</p>',
                    'content_1_2' => '',
                ]
            ]
        ],
        'global' => [
            'brand_navigation' => [
                'nanolash' => [
                    'title' => 'POZNAJ NASZE PRODUKTY DO RZĘS',
                    'url' => 'https://nanolash.pl',
                    'alt' => 'nanolash',
                    
                ],
                'nanobrow' => [
                    'title' => 'POZNAJ NASZE PRODUKTY DO BRWI',
                    'url' => 'https://nanobrow.pl',
                    'alt' => 'nanobrow',
                ]
            ],
            'cart' => [
                'add_to_cart' => 'Dodaj do koszyka',
            ],
            'slider' => [
                'next' => 'Następny slide',
                'prev' => 'Poprzedni slide',
                'dot' => 'Przejdź do slajdu',
                'nav' => 'Nawigacja po slajdach'
            ],
            'more' => 'Dowiedź się więcej',
            'socials' => [
                'facebook' => [
                    'alt' => 'facebook',
                    'link' => 'https://facebook.com',
                ],
                'instagram' => [
                    'alt' => 'instagram',
                    'link' => 'https://instagram.com',
                ],
                'pinterest' => [
                    'alt' => 'pinterest',
                    'link' => 'https://pinterest.com',
                ],
                'tiktok' => [
                    'alt' => 'tiktok',
                    'link' => 'https://tiktok.com',
                ],
                'youtube' => [
                    'alt' => 'youtube',
                    'link' => 'https://youtube.com',
                ],
            ],

            
        ],
        'about' => [],
        'contact' => [],
    ];